...
import { buildPrdComplianceCategoryPrompt } from "./prompts";
...
import { PRD_COMPLIANCE_REVIEW_CATEGORY_KEYS } from "@/lib/types/database";
import type { PrdComplianceReview, PrdComplianceReviewCategoryKey, PrdComplianceReviewFinding, PRD } from "@/lib/types/database";
...
export type PrdComplianceReviewStartResult =
  | { status: "started"; reviewId: string }
  | { status: "already_generating" }
  | { status: "error"; message: string };

export type PrdComplianceReviewStageResult = { status: "started" } | { status: "already_generating" };

async function releaseReviewAsFailed(supabase: SupabaseClient, reviewId: string, message: string): Promise<void> {
  await supabase.from("prd_compliance_reviews").update({ status: "failed", last_error: message }).eq("id", reviewId);
}
...
async function releaseCategoryAsFailed(
  ...
  await supabase
    .from("prd_compliance_review_categories")
    .update({ status: "failed", last_error: message })
    .eq("prd_compliance_review_id", reviewId)
...
/**
 * PRD Compliance Audit Engine (EngQA-Merge) — محرك مستقل تمامًا عن باقي
 * محركات Engineering QA (Single Responsibility). نفس بنية Database
 * Integrity Engine (Phase 12.3)، بفرق واحد: كل محور بيحتاج محتوى PRD
 * الفعلي (مش الكود بس) عشان يقارن بيه — راجع lib/prd-compliance-review/prompts.ts.
 */
export class PrdComplianceReviewEngine {
  static async startReview(
  ...
  ): Promise<PrdComplianceReviewStartResult> {
    ...
    if (active) {
      ...
      await supabase.from("prd_compliance_reviews").update({ status: "failed", last_error: "توقّفت المراجعة من غير سبب واضح (تجاوز الوقت المسموح)." }).eq("id", active.id);
    }
  ...
  static async runInitCore(reviewId: string): Promise<void> {
    ...
    try {
      const { data: review } = await supabase.from("prd_compliance_reviews").select("*").eq("id", reviewId).maybeSingle();
      if (!review) return;
      const r = review as PrdComplianceReview;
      ...
      const prd = await getPrdOrNull(supabase, r.project_id);
      if (!prd || !prd.overview.trim()) {
        await releaseReviewAsFailed(supabase, reviewId, "لا يوجد PRD معتمد لهذا المشروع — أنشئه أولاً من تبويب PRD.");
        return;
      }
      ...
      let carriedFindings: PrdComplianceReviewFinding[] = [];
      if (previousReady && !diff.isFullAnalysis) {
        ...
        const { data: previousFindings } = await supabase
          ...
          .select("*")
          .eq("prd_compliance_review_id", previousReady.id);
        carriedFindings = ((previousFindings ?? []) as PrdComplianceReviewFinding[]).filter((f) => !changedOrRemoved.has(f.file_path));
      }
      if (carriedFindings.length > 0) {
        await supabase.from("prd_compliance_review_findings").insert(
          carriedFindings.map((f) => ({
            prd_compliance_review_id: reviewId,
      ...
      if (fileRows.length > 0) await supabase.from("prd_compliance_review_files").insert(fileRows);
      ...
      await supabase
        ...
        .update({
          ...
          category_count: PRD_COMPLIANCE_REVIEW_CATEGORY_KEYS.length,
        })
        .eq("id", reviewId);
      ...
      if (diff.changedFiles.length === 0) {
        if (previousReady) {
          ...
          await PrdComplianceReviewEngine.finalizeReview(reviewId);
          return;
        }
      ...
      const categoryRows: { static_key: PrdComplianceReviewCategoryKey; skip: boolean }[] = PRD_COMPLIANCE_REVIEW_CATEGORY_KEYS.map((key) => ({
        static_key: key,
      ...
      for (const { static_key, skip } of categoryRows) {
        if (!skip) {
          await supabase.from("prd_compliance_review_categories").insert({
            prd_compliance_review_id: reviewId,
            project_id: r.project_id,
        ...
        const toInsert = ((prevFindings ?? []) as PrdComplianceReviewFinding[]).filter((f) => !alreadyCarried.has(f.finding_key));
      ...
      const { data: categoriesAfterInit } = await supabase.from("prd_compliance_review_categories").select("status").eq("prd_compliance_review_id", reviewId);
      const allReady = (categoriesAfterInit ?? []).length === PRD_COMPLIANCE_REVIEW_CATEGORY_KEYS.length && (categoriesAfterInit ?? []).every((c) => c.status === "ready");
      if (allReady) await PrdComplianceReviewEngine.finalizeReview(reviewId);
    } catch (err) {
      console.error(`[PrdComplianceReview] Unexpected init failure for review ${reviewId}:`, err);
      await releaseReviewAsFailed(supabase, reviewId, err instanceof Error ? err.message : "حدث خطأ غير متوقع أثناء التهيئة.");
    }
  }

  static async tryClaimCategoryLock(reviewId: string, categoryKey: PrdComplianceReviewCategoryKey): Promise<boolean> {
    const supabase = createServiceClient();
  ...
  static async runCategoryCore(reviewId: string, categoryKey: PrdComplianceReviewCategoryKey): Promise<void> {
    const supabase = createServiceClient();
    ...
    try {
      ...
      const prd = await getPrdOrNull(supabase, r.project_id);
      if (!prd) {
        await releaseCategoryAsFailed(supabase, reviewId, categoryKey, "لا يوجد PRD معتمد لهذا المشروع.");
        return;
      }

      const prompt = buildPrdComplianceCategoryPrompt(categoryKey, changedFiles, r.file_tree, r.is_incremental, prd);
      const response = await executeAIWithRateLimitWait(
        AITaskType.PRD_COMPLIANCE_REVIEW_CATEGORY,
        prompt,
      ...
      if (!validation.ok) {
        console.error(`[PrdComplianceReview] Category validation failed (${categoryKey}) review ${reviewId}: ${truncateForLog(validation.reason)}`);
      ...
      if (droppedCount > 0) {
        console.warn(`[PrdComplianceReview] Dropped ${droppedCount} ungrounded finding(s) for category ${categoryKey}, review ${reviewId}`);
      }
      ...
      const { data: allCategoryFindings } = await supabase
        ...
        .eq("prd_compliance_review_id", reviewId)
        .eq("category_key", categoryKey);
      const score = computePhaseCategoryScore((allCategoryFindings ?? []) as { severity: PrdComplianceReviewFinding["severity"] }[]);
      ...
      const { data: categories } = await supabase.from("prd_compliance_review_categories").select("status").eq("prd_compliance_review_id", reviewId);
      const allReady = (categories ?? []).length === PRD_COMPLIANCE_REVIEW_CATEGORY_KEYS.length && (categories ?? []).every((c) => c.status === "ready");
      if (allReady) await PrdComplianceReviewEngine.finalizeReview(reviewId);
    } catch (err) {
      console.error(`[PrdComplianceReview] Unexpected category failure (${categoryKey}) review ${reviewId}:`, err);
  ...
  private static async finalizeReview(reviewId: string): Promise<void> {
    ...
    const { data: categories } = await supabase.from("prd_compliance_review_categories").select("category_key, score").eq("prd_compliance_review_id", reviewId);
    const categoryScores = Object.fromEntries((categories ?? []).map((c) => [c.category_key, c.score ?? 100])) as Record<PrdComplianceReviewCategoryKey, number>;
    const scoreSummary = computePrdComplianceScoreSummary(categoryScores);

    const { data: findings } = await supabase.from("prd_compliance_review_findings").select("file_path").eq("prd_compliance_review_id", reviewId);
    const countByFile = new Map<string, number>();
    for (const f of findings ?? []) countByFile.set(f.file_path, (countByFile.get(f.file_path) ?? 0) + 1);
    const { data: fileRows } = await supabase.from("prd_compliance_review_files").select("id, file_path").eq("prd_compliance_review_id", reviewId);
    for (const row of fileRows ?? []) {
      await supabase.from("prd_compliance_review_files").update({ findings_count: countByFile.get(row.file_path) ?? 0 }).eq("id", row.id);
    }
  ...
  static async getState(projectId: string) {
    const supabase = createServiceClient();
    const { data: reviews } = await supabase.from("prd_compliance_reviews").select("*").eq("project_id", projectId).order("version", { ascending: false });
    const reviewList = (reviews ?? []) as PrdComplianceReview[];
  ...
  static async getReviewWithFindings(reviewId: string): Promise<{ review: PrdComplianceReview; findings: PrdComplianceReviewFinding[] } | null> {
    const supabase = createServiceClient();
    const { data: review } = await supabase.from("prd_compliance_reviews").select("*").eq("id", reviewId).maybeSingle();
    if (!review) return null;
    return { review: review as PrdComplianceReview, findings: await fetchFindings(supabase, reviewId) };
  }
}
...
async function copyPreviousCategoriesVerbatim(supabase: SupabaseClient, reviewId: string, projectId: string, previousReviewId: string): Promise<void> {
  const { data: previousCategories } = await supabase
    ...
    .eq("prd_compliance_review_id", previousReviewId);
  ...
  if (rows.length > 0) await supabase.from("prd_compliance_review_categories").insert(rows);
}

async function fetchCategories(supabase: SupabaseClient, reviewId: string) {
  const { data } = await supabase.from("prd_compliance_review_categories").select("*").eq("prd_compliance_review_id", reviewId).order("category_key", { ascending: true });
  return data ?? [];
}

async function fetchFindings(supabase: SupabaseClient, reviewId: string): Promise<PrdComplianceReviewFinding[]> {
  const { data } = await supabase.from("prd_compliance_review_findings").select("*").eq("prd_compliance_review_id", reviewId).order("severity", { ascending: true });
  return (data ?? []) as PrdComplianceReviewFinding[];
}

async function fetchFiles(supabase: SupabaseClient, reviewId: string) {
  const { data } = await supabase.from("prd_compliance_review_files").select("*").eq("prd_compliance_review_id", reviewId).order("file_path", { ascending: true });
  return data ?? [];
}
