...
import { buildDatabaseCategoryPrompt } from "./prompts";
import { isCategoryRelevantToChangedFiles, isFileRelevantToDatabaseCategory, isFileContentRelevantToQueryCategory } from "./relevance";
...
import { DATABASE_REVIEW_CATEGORY_KEYS } from "@/lib/types/database";
import type { DatabaseReview, DatabaseReviewCategoryKey, DatabaseReviewFinding } from "@/lib/types/database";

/** ميزانية انتظار الـ Rate Limit لكل محور — نفس قيمة Static/Security Review، تحت سقف maxDuration=260s بهامش أمان. */
const CATEGORY_RATE_LIMIT_BUDGET_MS = 170_000;

export type DatabaseReviewStartResult =
  | { status: "started"; reviewId: string }
  | { status: "already_generating" }
  | { status: "error"; message: string };

export type DatabaseReviewStageResult = { status: "started" } | { status: "already_generating" };

async function releaseReviewAsFailed(supabase: SupabaseClient, reviewId: string, message: string): Promise<void> {
  await supabase.from("database_reviews").update({ status: "failed", last_error: message }).eq("id", reviewId);
}
...
async function releaseCategoryAsFailed(
  ...
  await supabase
    .from("database_review_categories")
    .update({ status: "failed", last_error: message })
    .eq("database_review_id", reviewId)
    .eq("category_key", categoryKey);
}

/**
 * Database Integrity Engine (Phase 12.3) — محرك مستقل تمامًا عن Security
 * Audit Engine (Single Responsibility). نفس بنية Static Architecture
 * Review Engine (Phase 12.2) بالضبط: Incremental عبر git diff، 6 محاور
 * مستقلة بالتسلسل، درجات حسابية بدل توليف AI، وتخطي المحاور مالهاش أي
 * ملف متغيّر مرتبط بيها.
 */
export class DatabaseReviewEngine {
  static async startReview(
  ...
  ): Promise<DatabaseReviewStartResult> {
    ...
    if (active) {
      ...
      await supabase.from("database_reviews").update({ status: "failed", last_error: "توقّفت المراجعة من غير سبب واضح (تجاوز الوقت المسموح)." }).eq("id", active.id);
    }
  ...
  static async runInitCore(reviewId: string): Promise<void> {
    ...
    try {
      const { data: review } = await supabase.from("database_reviews").select("*").eq("id", reviewId).maybeSingle();
      if (!review) return;
      const r = review as DatabaseReview;
      ...
      let carriedFindings: DatabaseReviewFinding[] = [];
      if (previousReady && !diff.isFullAnalysis) {
        ...
        const { data: previousFindings } = await supabase
          ...
          .select("*")
          .eq("database_review_id", previousReady.id);
        carriedFindings = ((previousFindings ?? []) as DatabaseReviewFinding[]).filter((f) => !changedOrRemoved.has(f.file_path));
      }
      if (carriedFindings.length > 0) {
        await supabase.from("database_review_findings").insert(
          carriedFindings.map((f) => ({
            database_review_id: reviewId,
      ...
      if (fileRows.length > 0) await supabase.from("database_review_files").insert(fileRows);
      ...
      await supabase
        ...
        .update({
          ...
          category_count: DATABASE_REVIEW_CATEGORY_KEYS.length,
        })
        .eq("id", reviewId);
      ...
      if (diff.changedFiles.length === 0) {
        if (previousReady) {
          ...
          await DatabaseReviewEngine.finalizeReview(reviewId);
          return;
        }
      ...
      const categoryRows: { static_key: DatabaseReviewCategoryKey; skip: boolean }[] = DATABASE_REVIEW_CATEGORY_KEYS.map((key) => ({
        static_key: key,
      ...
      for (const { static_key, skip } of categoryRows) {
        if (!skip) {
          await supabase.from("database_review_categories").insert({
            database_review_id: reviewId,
            project_id: r.project_id,
        ...
        const toInsert = ((prevFindings ?? []) as DatabaseReviewFinding[]).filter((f) => !alreadyCarried.has(f.finding_key));
      ...
      const { data: categoriesAfterInit } = await supabase.from("database_review_categories").select("status").eq("database_review_id", reviewId);
      const allReady = (categoriesAfterInit ?? []).length === DATABASE_REVIEW_CATEGORY_KEYS.length && (categoriesAfterInit ?? []).every((c) => c.status === "ready");
      if (allReady) await DatabaseReviewEngine.finalizeReview(reviewId);
    } catch (err) {
      console.error(`[DatabaseReview] Unexpected init failure for review ${reviewId}:`, err);
      await releaseReviewAsFailed(supabase, reviewId, err instanceof Error ? err.message : "حدث خطأ غير متوقع أثناء التهيئة.");
    }
  }

  static async tryClaimCategoryLock(reviewId: string, categoryKey: DatabaseReviewCategoryKey): Promise<boolean> {
    const supabase = createServiceClient();
  ...
  static async runCategoryCore(reviewId: string, categoryKey: DatabaseReviewCategoryKey): Promise<void> {
    const supabase = createServiceClient();
    ...
    try {
      ...
      // زي إصلاح lib/security-review/generation-service.ts بالظبط: كل
      // محور كان بياخد نفس مجموعة changedFiles كاملة كما هي، حتى لو
      // نطاقه ضيق (database_structure/data_integrity بيهتموا بملفات
      // migrations فقط). محور query خاص: نطاقه بالمسار "أي ملف" فعلاً
      // (relevance.ts بيرجّع null قصدًا)، فالفلترة هنا بمحتوى الملف
      // (فيه استدعاء query فعلي؟) مش مساره — بدل ما ياخد الـ Repository
      // كله زي ما كان بيحصل.
      const relevantFiles =
        categoryKey === "query"
          ? changedFiles.filter((f) => isFileContentRelevantToQueryCategory(f.content))
          : changedFiles.filter((f) => isFileRelevantToDatabaseCategory(categoryKey, f.path));
      const filesForPrompt = relevantFiles.length > 0 ? relevantFiles : changedFiles;

      const prompt = buildDatabaseCategoryPrompt(categoryKey, filesForPrompt, r.file_tree, r.is_incremental);
      const response = await executeAIWithRateLimitWait(
        AITaskType.DATABASE_REVIEW_CATEGORY,
        prompt,
      ...
      if (!validation.ok) {
        console.error(`[DatabaseReview] Category validation failed (${categoryKey}) review ${reviewId}: ${truncateForLog(validation.reason)}`);
      ...
      if (droppedCount > 0) {
        console.warn(`[DatabaseReview] Dropped ${droppedCount} ungrounded finding(s) for category ${categoryKey}, review ${reviewId}`);
      }
      ...
      const { data: allCategoryFindings } = await supabase
        ...
        .eq("database_review_id", reviewId)
        .eq("category_key", categoryKey);
      const score = computePhaseCategoryScore((allCategoryFindings ?? []) as { severity: DatabaseReviewFinding["severity"] }[]);
      ...
      const { data: categories } = await supabase.from("database_review_categories").select("status").eq("database_review_id", reviewId);
      const allReady = (categories ?? []).length === DATABASE_REVIEW_CATEGORY_KEYS.length && (categories ?? []).every((c) => c.status === "ready");
      if (allReady) await DatabaseReviewEngine.finalizeReview(reviewId);
    } catch (err) {
      console.error(`[DatabaseReview] Unexpected category failure (${categoryKey}) review ${reviewId}:`, err);
  ...
  private static async finalizeReview(reviewId: string): Promise<void> {
    ...
    const { data: categories } = await supabase.from("database_review_categories").select("category_key, score").eq("database_review_id", reviewId);
    const categoryScores = Object.fromEntries((categories ?? []).map((c) => [c.category_key, c.score ?? 100])) as Record<DatabaseReviewCategoryKey, number>;
    const scoreSummary = computeDatabaseScoreSummary(categoryScores);

    const { data: findings } = await supabase.from("database_review_findings").select("file_path").eq("database_review_id", reviewId);
    const countByFile = new Map<string, number>();
    for (const f of findings ?? []) countByFile.set(f.file_path, (countByFile.get(f.file_path) ?? 0) + 1);
    const { data: fileRows } = await supabase.from("database_review_files").select("id, file_path").eq("database_review_id", reviewId);
    for (const row of fileRows ?? []) {
      await supabase.from("database_review_files").update({ findings_count: countByFile.get(row.file_path) ?? 0 }).eq("id", row.id);
    }
  ...
  static async getState(projectId: string) {
    const supabase = createServiceClient();
    const { data: reviews } = await supabase.from("database_reviews").select("*").eq("project_id", projectId).order("version", { ascending: false });
    const reviewList = (reviews ?? []) as DatabaseReview[];
  ...
  static async getReviewWithFindings(reviewId: string): Promise<{ review: DatabaseReview; findings: DatabaseReviewFinding[] } | null> {
    const supabase = createServiceClient();
    const { data: review } = await supabase.from("database_reviews").select("*").eq("id", reviewId).maybeSingle();
    if (!review) return null;
    return { review: review as DatabaseReview, findings: await fetchFindings(supabase, reviewId) };
  }
}
...
async function copyPreviousCategoriesVerbatim(supabase: SupabaseClient, reviewId: string, projectId: string, previousReviewId: string): Promise<void> {
  const { data: previousCategories } = await supabase
    ...
    .eq("database_review_id", previousReviewId);
  ...
  if (rows.length > 0) await supabase.from("database_review_categories").insert(rows);
}

async function fetchCategories(supabase: SupabaseClient, reviewId: string) {
  const { data } = await supabase.from("database_review_categories").select("*").eq("database_review_id", reviewId).order("category_key", { ascending: true });
  return data ?? [];
}

async function fetchFindings(supabase: SupabaseClient, reviewId: string): Promise<DatabaseReviewFinding[]> {
  const { data } = await supabase.from("database_review_findings").select("*").eq("database_review_id", reviewId).order("severity", { ascending: true });
  return (data ?? []) as DatabaseReviewFinding[];
}

async function fetchFiles(supabase: SupabaseClient, reviewId: string) {
  const { data } = await supabase.from("database_review_files").select("*").eq("database_review_id", reviewId).order("file_path", { ascending: true });
  return data ?? [];
}
