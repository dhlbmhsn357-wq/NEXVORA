...
import { buildArchitectureCategoryPrompt } from "./prompts";
...
import { ARCHITECTURE_REVIEW_CATEGORY_KEYS } from "@/lib/types/database";
import type { ArchitectureReview, ArchitectureReviewCategoryKey, ArchitectureReviewFinding } from "@/lib/types/database";
...
export type ArchitectureReviewStartResult =
  | { status: "started"; reviewId: string }
  | { status: "already_generating" }
  | { status: "error"; message: string };

export type ArchitectureReviewStageResult = { status: "started" } | { status: "already_generating" };

async function releaseReviewAsFailed(supabase: SupabaseClient, reviewId: string, message: string): Promise<void> {
  await supabase.from("architecture_reviews").update({ status: "failed", last_error: message }).eq("id", reviewId);
}
...
async function releaseCategoryAsFailed(
  ...
  await supabase
    .from("architecture_review_categories")
...
/**
 * Architecture Audit Engine (EngQA-Merge) — محرك مستقل تمامًا عن باقي
 * محركات Engineering QA (Single Responsibility). نفس بنية Database
 * Integrity Engine (Phase 12.3) بالضبط: Incremental عبر git diff، 6
 * محاور مستقلة بالتسلسل، درجات حسابية بدل توليف AI، وتخطي المحاور
 * مالهاش أي ملف متغيّر مرتبط بيها.
 */
export class ArchitectureReviewEngine {
  static async startReview(
  ...
  ): Promise<ArchitectureReviewStartResult> {
    ...
    if (active) {
      ...
      await supabase.from("architecture_reviews").update({ status: "failed", last_error: "توقّفت المراجعة من غير سبب واضح (تجاوز الوقت المسموح)." }).eq("id", active.id);
    }
  ...
  static async runInitCore(reviewId: string): Promise<void> {
    ...
    try {
      const { data: review } = await supabase.from("architecture_reviews").select("*").eq("id", reviewId).maybeSingle();
      if (!review) return;
      const r = review as ArchitectureReview;
      ...
      let carriedFindings: ArchitectureReviewFinding[] = [];
      if (previousReady && !diff.isFullAnalysis) {
        ...
        const { data: previousFindings } = await supabase
          ...
          .select("*")
          .eq("architecture_review_id", previousReady.id);
        carriedFindings = ((previousFindings ?? []) as ArchitectureReviewFinding[]).filter((f) => !changedOrRemoved.has(f.file_path));
      }
      if (carriedFindings.length > 0) {
        await supabase.from("architecture_review_findings").insert(
      ...
      if (fileRows.length > 0) await supabase.from("architecture_review_files").insert(fileRows);
      ...
      await supabase
        ...
        .update({
          ...
          category_count: ARCHITECTURE_REVIEW_CATEGORY_KEYS.length,
        })
        .eq("id", reviewId);
      ...
      if (diff.changedFiles.length === 0) {
        if (previousReady) {
          ...
          await ArchitectureReviewEngine.finalizeReview(reviewId);
          return;
        }
      ...
      const categoryRows: { static_key: ArchitectureReviewCategoryKey; skip: boolean }[] = ARCHITECTURE_REVIEW_CATEGORY_KEYS.map((key) => ({
        static_key: key,
      ...
      for (const { static_key, skip } of categoryRows) {
        if (!skip) {
          await supabase.from("architecture_review_categories").insert({
            architecture_review_id: reviewId,
            project_id: r.project_id,
        ...
        const toInsert = ((prevFindings ?? []) as ArchitectureReviewFinding[]).filter((f) => !alreadyCarried.has(f.finding_key));
      ...
      const { data: categoriesAfterInit } = await supabase.from("architecture_review_categories").select("status").eq("architecture_review_id", reviewId);
      const allReady = (categoriesAfterInit ?? []).length === ARCHITECTURE_REVIEW_CATEGORY_KEYS.length && (categoriesAfterInit ?? []).every((c) => c.status === "ready");
      if (allReady) await ArchitectureReviewEngine.finalizeReview(reviewId);
    } catch (err) {
      console.error(`[ArchitectureReview] Unexpected init failure for review ${reviewId}:`, err);
      await releaseReviewAsFailed(supabase, reviewId, err instanceof Error ? err.message : "حدث خطأ غير متوقع أثناء التهيئة.");
    }
  }

  static async tryClaimCategoryLock(reviewId: string, categoryKey: ArchitectureReviewCategoryKey): Promise<boolean> {
    const supabase = createServiceClient();
  ...
  static async runCategoryCore(reviewId: string, categoryKey: ArchitectureReviewCategoryKey): Promise<void> {
    const supabase = createServiceClient();
    ...
    try {
      ...
      const prompt = buildArchitectureCategoryPrompt(categoryKey, changedFiles, r.file_tree, r.is_incremental);
      const response = await executeAIWithRateLimitWait(
        AITaskType.ARCHITECTURE_REVIEW_CATEGORY,
        prompt,
      ...
      if (!validation.ok) {
        console.error(`[ArchitectureReview] Category validation failed (${categoryKey}) review ${reviewId}: ${truncateForLog(validation.reason)}`);
      ...
      if (droppedCount > 0) {
        console.warn(`[ArchitectureReview] Dropped ${droppedCount} ungrounded finding(s) for category ${categoryKey}, review ${reviewId}`);
      }
      ...
      const { data: allCategoryFindings } = await supabase
        ...
        .eq("architecture_review_id", reviewId)
        .eq("category_key", categoryKey);
      const score = computePhaseCategoryScore((allCategoryFindings ?? []) as { severity: ArchitectureReviewFinding["severity"] }[]);
      ...
      const { data: categories } = await supabase.from("architecture_review_categories").select("status").eq("architecture_review_id", reviewId);
      const allReady = (categories ?? []).length === ARCHITECTURE_REVIEW_CATEGORY_KEYS.length && (categories ?? []).every((c) => c.status === "ready");
      if (allReady) await ArchitectureReviewEngine.finalizeReview(reviewId);
    } catch (err) {
      console.error(`[ArchitectureReview] Unexpected category failure (${categoryKey}) review ${reviewId}:`, err);
  ...
  private static async finalizeReview(reviewId: string): Promise<void> {
    ...
    const { data: categories } = await supabase.from("architecture_review_categories").select("category_key, score").eq("architecture_review_id", reviewId);
    const categoryScores = Object.fromEntries((categories ?? []).map((c) => [c.category_key, c.score ?? 100])) as Record<ArchitectureReviewCategoryKey, number>;
    const scoreSummary = computeArchitectureScoreSummary(categoryScores);

    const { data: findings } = await supabase.from("architecture_review_findings").select("file_path").eq("architecture_review_id", reviewId);
    const countByFile = new Map<string, number>();
    for (const f of findings ?? []) countByFile.set(f.file_path, (countByFile.get(f.file_path) ?? 0) + 1);
    const { data: fileRows } = await supabase.from("architecture_review_files").select("id, file_path").eq("architecture_review_id", reviewId);
    for (const row of fileRows ?? []) {
      await supabase.from("architecture_review_files").update({ findings_count: countByFile.get(row.file_path) ?? 0 }).eq("id", row.id);
    }
  ...
  static async getState(projectId: string) {
    const supabase = createServiceClient();
    const { data: reviews } = await supabase.from("architecture_reviews").select("*").eq("project_id", projectId).order("version", { ascending: false });
    const reviewList = (reviews ?? []) as ArchitectureReview[];
  ...
  static async getReviewWithFindings(reviewId: string): Promise<{ review: ArchitectureReview; findings: ArchitectureReviewFinding[] } | null> {
    const supabase = createServiceClient();
    const { data: review } = await supabase.from("architecture_reviews").select("*").eq("id", reviewId).maybeSingle();
    if (!review) return null;
    return { review: review as ArchitectureReview, findings: await fetchFindings(supabase, reviewId) };
  }
}
...
async function copyPreviousCategoriesVerbatim(supabase: SupabaseClient, reviewId: string, projectId: string, previousReviewId: string): Promise<void> {
  const { data: previousCategories } = await supabase
    ...
    .eq("architecture_review_id", previousReviewId);
  ...
  if (rows.length > 0) await supabase.from("architecture_review_categories").insert(rows);
}

async function fetchCategories(supabase: SupabaseClient, reviewId: string) {
  const { data } = await supabase.from("architecture_review_categories").select("*").eq("architecture_review_id", reviewId).order("category_key", { ascending: true });
  return data ?? [];
}

async function fetchFindings(supabase: SupabaseClient, reviewId: string): Promise<ArchitectureReviewFinding[]> {
  const { data } = await supabase.from("architecture_review_findings").select("*").eq("architecture_review_id", reviewId).order("severity", { ascending: true });
  return (data ?? []) as ArchitectureReviewFinding[];
}

async function fetchFiles(supabase: SupabaseClient, reviewId: string) {
  const { data } = await supabase.from("architecture_review_files").select("*").eq("architecture_review_id", reviewId).order("file_path", { ascending: true });
  return data ?? [];
}
